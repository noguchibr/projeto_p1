...
def find_centroid(polygon):
    ...
    i, soma1, latitudeLista, longitudeLista = 0, [], [], []
    tamPoligono = len(polygon) - 1
    while i < tamPoligono:
        multComum = latitude(polygon[i]) * longitude(polygon[i + 1]) - latitude(polygon[i + 1]) * longitude(polygon[i])
        soma1.append(multComum)
        latitudeLista.append((latitude(polygon[i]) + latitude(polygon[i + 1])) * multComum)
        longitudeLista.append((longitude(polygon[i]) + longitude(polygon[i + 1])) * multComum)
        i = i + 1
    area = 0.5 * (sum(soma1))
    if area == 0:
        return latitude(polygon[0]), longitude(polygon[0]), 0
    x = sum(latitudeLista) / (6 * area)
    y = sum(longitudeLista) / (6 * area)
    return x, y, abs(area)
...
def find_center(polygons):
    ...
    if len(polygons) == 1:
        ...
        #centroEstados = []
        latDividendo, longiDividendo, divisor = 0, 0, 0

        for estado in polygons:
            #print(estado)

            centroide = find_centroid(estado)

            latDividendo = latDividendo + centroide[0]*centroide[2]
            longiDividendo = longiDividendo + centroide[1]*centroide[2]

            divisor = divisor + centroide[2]

            #centroEstados.append(find_centroid(estado))

        latCentro =  latDividendo/divisor
        longiCentro = longiDividendo/divisor

        return latCentro, longiCentro
        #return centroEstados
...
def average_sentiments(tweets_by_state):
    ...
    for key in tweets_by_state.keys():
        tweetsPorEstado = tweets_by_state[key] 
        contador = 0 
        for x in range(len(tweetsPorEstado)): 
            if has_sentiment(analyze_tweet_sentiment(tweetsPorEstado[x])) == False: 
                tweetsPorEstado[x] = 0
            else:
                contador += 1 
                tweetsPorEstado[x] = sentiment_value(analyze_tweet_sentiment(tweetsPorEstado[x])) 
        if contador != 0: 
            averaged_state_sentiments[key] = sum(tweetsPorEstado) / (contador) 
...
def group_tweets_by_hour(tweets):
    ...
    "*** YOUR CODE HERE ***"
    for tweet in tweets:
        if tweet_time(tweet).hour in tweets_by_hour:
